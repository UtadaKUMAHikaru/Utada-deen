...
class ResponseClient:
    def __init__(self, utada_deen, model="", characters={}):
        self.utada_deen = utada_deen
        self.characters = characters
        self.continue_talking = False
        self.allow_mentions = True
        self.save_counter = 0
        self.model = model if model else utada_deen.run_name
    ...
    async def get_history(self, message):
        if "sysadmin" in self.utada_deen.run_name:
    ...
    async def trump_or_ai(self, message):        
        ...
        async with message.channel.typing():
            ...
            if self.last_message == "trump":
                ...
                response = self.utada_deen.start_conversation([("Donald Trump", response)], run_name=self.model, character="Donald Trump", characters=list(self.characters.keys())).strip()
    ...
    async def breakdown_character(self, message):
        ...
        await message.channel.send(self.utada_deen.get_emotional_profile(character, f"history/{message.guild.name}_{message.channel.name}.txt"))
    ...
    async def show_models(self, message):
        ...
        await message.channel.send(f"Currently using: {self.utada_deen.run_name}\nAvailable:\n\t{available_models}")

    async def modify_temperature(self, message):
        temperature = self.get_parameter_string(message, "!temperature")
        if temperature:
            try:
                self.utada_deen.temperature = float(temperature)
            except:
                pass
        await message.channel.send(f"Temperature: {self.utada_deen.temperature}")
    ...
    async def respond(self, history, message, character):
        async with message.channel.typing():
            response = self.utada_deen.start_conversation(history, run_name=self.model, character=character, characters=list(self.characters.keys())).strip()
            if not response:
                response = self.utada_deen.start_conversation(history, random_seed=True, run_name=self.model, character=character, characters=list(self.characters.keys())).strip()
            for member in message.guild.members:
